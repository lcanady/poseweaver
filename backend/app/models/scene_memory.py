...
class FlagType(str, Enum):
    """Continuity flag type enumeration."""
...
class ContinuityFlag(BaseModel):
    """
    Continuity flags for tracking potential issues.
    
    Identifies and tracks potential inconsistencies in character behavior,
    environmental details, timeline, plot elements, and relationships.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.flag_type = FlagType(flag_type) if isinstance(flag_type, str) else flag_type
        self.description = description
        self.severity = Severity(severity) if isinstance(severity, str) else severity
        self.resolved = resolved
        self.resolution_notes = resolution_notes
        self.confidence_score = max(0.0, min(1.0, confidence_score))  # Clamp between 0 and 1
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'ContinuityFlag':
        """Create from dictionary."""
        if not data:
            return None
        return cls(**data)
    ...
    @classmethod
    def find_by_pose(cls, pose_id: str) -> List['ContinuityFlag']:
        """Find continuity flags by pose ID."""
        return cls.find_all(query={'pose_id': pose_id})
    
    @classmethod
    def find_unresolved(cls, pose_id: Optional[str] = None) -> List['ContinuityFlag']:
        """Find unresolved continuity flags."""
        query = {'resolved': False}
        if pose_id:
            query['pose_id'] = pose_id
        return cls.find_all(query=query, sort=[('severity', -1), ('created_at', -1)])
    
    @classmethod
    def find_by_scene(cls, scene_id: str, resolved: Optional[bool] = None) -> List['ContinuityFlag']:
        """Find continuity flags for poses in a specific scene."""
        # First get all poses in the scene
        poses = Pose.find_by_scene(scene_id)
        pose_ids = [pose.id for pose in poses if pose.id]
        
        if not pose_ids:
            return []
        
        query = {'pose_id': {'$in': pose_ids}}
    ...
    @classmethod
    def find_by_severity(cls, severity: Severity, resolved: bool = False) -> List['ContinuityFlag']:
        """Find continuity flags by severity level."""
        return cls.find_all(
            query={'severity': severity.value, 'resolved': resolved},
            sort=[('created_at', -1)]
        )
    
    @classmethod
    def initialize_indexes(cls) -> None:
        """Initialize database indexes for continuity flags."""
        from ..services.mongodb_service import get_mongodb_service
        mongodb = get_mongodb_service()
        
        # Create indexes
        mongodb.create_index(cls.COLLECTION_NAME, 'pose_id')
        mongodb.create_index(cls.COLLECTION_NAME, 'flag_type')
        mongodb.create_index(cls.COLLECTION_NAME, 'severity')
        mongodb.create_index(cls.COLLECTION_NAME, 'resolved')
        mongodb.create_index(cls.COLLECTION_NAME, 'created_at')