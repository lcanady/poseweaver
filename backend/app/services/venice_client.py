...
class VeniceClient:
    ...
    def generate_completion(
        ...
        if response_format is not None:
            # Ensure proper format according to Venice API requirements
            if "type" in response_format and response_format["type"] == "json_schema":
                # Enforce required settings for Venice structured responses
        ...
        while retry_count <= max_retries:
            try:
                ...
                print(f"[DEBUG] Venice API Request:")
                ...
                # If we get a 400 error and thinking is enabled, try without thinking
                if e.response.status_code == 400 and thinking and "thinking" in data:
                    print(f"[DEBUG] HTTP 400 with thinking parameter, retrying without thinking...")
                    data_without_thinking = data.copy()
                    del data_without_thinking["thinking"]
                    ...
                    try:
                        ...
                        raise VeniceAPIError(f"Request failed even without thinking parameter: {str(fallback_error)}")
                else:
                    raise VeniceAPIError(f"HTTP {e.response.status_code}: {str(e)}")
            except Timeout as e:
                retry_count += 1
                if retry_count > max_retries:
                    raise VeniceAPIError(f"Request timeout after {max_retries} retries: {str(e)}")
                else:
    ...
    def extract_structured_data(self, 
        ...
        # Create a Venice structured response format JSON schema
        properties = {}
        required_fields = []
        ...
        try:
            ...
            raise VeniceAPIError(f"Error extracting structured data: {str(e)}. Response: {response if 'response' in locals() else 'No response'}")