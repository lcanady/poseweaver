...
class AIClient:
    """Generic client for interacting with AI Providers (OpenRouter, etc)."""
    
    def __init__(self, api_key: str = None, base_url: str = None):
        """Initialize the AI client.
        
        Args:
            api_key: AI Provider API key (defaults to env AI_API_KEY)
            base_url: Base URL for the API (defaults to env AI_BASE_URL)
        """
        import os
        self.api_key = api_key or os.getenv('AI_API_KEY') or os.getenv('OPENROUTER_API_KEY')
        self.base_url = base_url or os.getenv('AI_BASE_URL', "https://openrouter.ai/api/v1")
        self.model = os.getenv('AI_MODEL', "cognitivecomputations/dolphin-mistral-24b-venice-edition:free")
        
        app_url = os.getenv('APP_URL', 'http://localhost:3000')
        app_name = os.getenv('APP_NAME', 'PoseWeaver')
        ...
        self.timeout = int(os.getenv('AI_TIMEOUT', '60'))  # seconds
    ...
    def generate_completion(
        ...
        """Generate a completion using AI Provider API.
        
        Args:
            prompt: The user prompt
            model: Model to use (defaults to env AI_MODEL)
